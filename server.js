...
// UPDATE SNIPPET (PATCH/PUT)
async function updateSnippetHandler(req, res) {
  try {
    const body = { ...req.body };
    ...
    const updated = await Snippet.findByIdAndUpdate(req.params.id, body, {
      new: true,
      runValidators: true,
    });
...
app.patch("/api/snippets/:id", updateSnippetHandler);
app.put("/api/snippets/:id", updateSnippetHandler);
